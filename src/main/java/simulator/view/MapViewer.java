...
import simulator.model.Animal; //habria que meterlo en Animal
...
public class MapViewer extends AbstractMapViewer {
			...
			public void keyPressed(KeyEvent e) {
				...
				case 's':
					...
					if (currentState == null) {
						currentState = Animal.State.values()[0];
					} else {
						int i = 0;
						while (i < Animal.State.values().length && Animal.State.values()[i] != currentState) {
							i++;
						}
						if (i == Animal.State.values().length - 1) {
							currentState = null;
						} else {
							currentState = Animal.State.values()[i + 1];
						}
					}
	...
	private boolean visible(AnimalInfo a) {
		...
		//
		if(currentState == null || a.getState() == currentState){
			return true;
		}
		else{
			return false;
	...
	private void drawObjects(Graphics2D g, Collection<AnimalInfo> animals, Double time) {
		...
		for (Entry<String, SpeciesInfo> e : kindsInfo.entrySet()) {
			...
			drawStringWithRect(g, 10, 580 - 20*i, String.format("%s: %d", e.getKey(), e.getValue().count));
			i++; 
			e.getValue().count = 0;
		}
	}
